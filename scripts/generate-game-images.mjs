 ...
 *
 * Uses curl for HTTP so it works in environments where Node's built-in
 * fetch cannot resolve external hostnames.
 */
...
const GAMES = [
  ...
  { id: 'right-time',          prompt: `A bright cheerful analog clock with bold colorful hands pointing to three twenty, ${STYLE}` },
  ...
  { id: 'snake-lite',          prompt: `A cute green pixel snake on a dark grid board chasing a bright red apple, retro game art style, ${STYLE}` },
...
function generateImage(game) {
  ...
  if (!b64) throw new Error(`No image data in response: ${raw.toString().slice(0, 200)}`);
